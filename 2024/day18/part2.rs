...
fn process_file(file_path: &str) -> io::Result<()> {
    ...
    let mut last_byte: Option<Position> = None;
    for (i, &byte) in bytes.iter().enumerate() {
        if last_byte.is_some() && i >= STEPS {
            // Construct the graph
            let g = construct_graph(&memory);
            let start = (0, 0);
            let goal = (memory[0].len() - 1, memory.len() - 1);
            let (came_from, _) = A_star(&g, start, goal);
            let result = get_path_length(&came_from, goal);
            if result == 0 {
                println!("Result: {},{}", last_byte.unwrap().0, last_byte.unwrap().1);
                break;
            }
        }
        let (x, y) = byte;
